/**
 * 逐条对照 minitool-zip-builder 规范，校验 dist/ 产物（通用版，适配多文件业务 JS）。
...
// 跨端 CSS 内容：index.html 内联 <style> + 包内 .css
const cssFiles = files.filter((f) => f.endsWith('.css')).map((f) => fs.readFileSync(path.join(DIST, f), 'utf8')).join('\n');
const htmlStyle = [...html.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)].map((m) => m[1]).join('\n');
...
const extCss = [...cssAll.matchAll(/url\(\s*['"]?(https?:\/\/[^)'"]+)/gi)].map((m) => m[1]);
...
// 经典脚本（无 module / import / export）
const moduleTag = [...html.matchAll(/<script[^>]*\stype\s*=\s*["']module["']/gi)].length;
moduleTag ? bad('脚本为经典脚本', moduleTag + ' 处 type="module"') : ok('脚本为经典脚本（无 type="module"）');
/(^|\n)\s*(import|export)\s/.test(allJs) ? bad('JS 无 import/export') : ok('JS 无 import/export（无模块语法）');
...
const bizHits = SCAN.filter(([r]) => r.test(allJs)).map(([, n]) => n);
bizHits.length ? bad('业务代码无被禁能力', bizHits.join(',')) : ok('业务代码无被禁能力（' + SCAN.length + ' 项模式全部零命中）');

/* ========== cross-platform-h5 §6 · 跨端自检 ========== */
/pointerdown|touchstart/.test(allJs) ? ok('交互用 pointer/touch events') : warn('交互用 pointer/touch events', '未检测到');

const hoverRules = [...cssAll.matchAll(/([^{}]*:hover[^{]*)\{([^}]*)\}/g)];
...
/safe-area-inset/.test(cssAll) ? ok('安全区适配 var(--safe-area-inset-*, env(...))') : bad('安全区适配');
/touch-action/.test(cssAll) ? ok('touch-action 已设置') : warn('touch-action 已设置');
/-webkit-tap-highlight-color/.test(cssAll) ? ok('tap-highlight 已处理') : warn('tap-highlight 已处理');
/overscroll-behavior/.test(cssAll) ? ok('滚动容器 overscroll-behavior') : warn('滚动容器 overscroll-behavior');

// 只算真正写死的 width，max-width / min-width / 媒体查询属响应式合理用法
const hardPx = [...cssAll.matchAll(/(^|[;{\s])width\s*:\s*(\d{3,})px/g)].map((m) => m[2]);
hardPx.length
  ? warn('无写死像素宽度', 'width:' + hardPx.join('px, ') + 'px')
  : ok('无写死像素宽度', 'max-width/min-width 属响应式用法，不计入');

// 布局自适应能力
const flexible = /max-width\s*:|flex\s*:|%|vw/.test(cssAll);
flexible ? ok('布局自适应（flex/%/max-width）') : warn('布局自适应');

/* ========== 体积 ========== */
const zipLimit = 10 * 1024 * 1024;
let rawTotal = 0;
for (const f of files) rawTotal += fs.statSync(path.join(DIST, f)).size;
rawTotal <= zipLimit ? ok('总包体积 < 10MB', (rawTotal / 1024 / 1024).toFixed(2) + ' MB') : bad('总包体积 < 10MB', (rawTotal / 1024 / 1024).toFixed(2) + ' MB');
