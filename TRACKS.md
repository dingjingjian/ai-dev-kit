...
├── reference/    # 非赛道：他人优秀作品归档（不参赛，不进下面四张表）
└── vibecoding-gallery/  # 非参赛：归档浏览工具（不是大赛作品，不计入赛道表）
```

`reference/` 与 `vibecoding-gallery/` **都不是赛道分类**，不参赛、不进下面四张表：
- `reference/<博主-工具>/` —— 归档的他人作品（截图 + 元信息），规范见 `reference/README.md`；
- `vibecoding-gallery/` —— 浏览上面这些归档的展示小工具（非参赛作品），说明见 `vibecoding-gallery/README.md`；它会自动定位 `reference/`，两种摆法都能跑。
...
| 项目 | 目录 | 一句话定位 | 物料状态 |
|------|------|-----------|----------|
| 准时下班热力图 | `vibetool/offwork-heatmap/` | 上下班打卡 + GitHub 式热力图看每日准时情况 | 已发布 |
| 心情日记 | `vibetool/mood-diary/` | 每天一种颜色记心情，GitHub 式热力图回看半年情绪 | 已发布 |
| 拼豆设计工具（拼豆神器） | `vibetool/perler-bead-designer/` | 48 色标准色卡的拼豆图纸设计与导出 | 已发布 |
| 像素画编辑器 | `vibetool/pixel-art-editor/` | 16–256 画布，笔刷/图层/撤销，导出 SVG+PNG | 已发布 |
| Word 转 Markdown | `vibetool/word-to-md/` | 浏览器端 .docx 转 Markdown | 自用 · 不投稿 |
...
| 中秋拼豆小游戏 | `vibegame/perler-mid-autumn/` | 中秋版拼豆：月夜纹样 + 猜灯谜（已含导出图纸） | 已发布 |
...
| 项目 | 目录 | 一句话定位 | 物料状态 |
|------|------|-----------|----------|
| 罗马军团图鉴 | `vibeknow/rome-total-war-3d/` | 古罗马时代阵营与兵种图鉴：8 阵营 × 6 兵种 = 48，阵营过渡页 + 逐队检阅（装备拆解 + 六维战力）+ 军团志，复用 jurassic-park-3d 框架 | 代码完成 · 素材齐备（装备图待抠透明底）· BGM 待换授权曲目 |
| 口袋地球 | `vibeknow/earth-3d/` | 昼夜交替、四季成因、月相、地球内部结构 | 完整（zip + 物料） |
| 寰宇小馆 | `vibeknow/world-food-3d/` | 复用 earth-3d 框架，35 道世界美食钉在地球上，点红点看介绍 | 已发布 |
...
| 全球交通工具图鉴 | `vibeknow/vehicle-atlas/` | 49 种交通工具「类型」速查，按地面 / 水面 / 天空 / 太空四类分组、类内按出现时间从早到晚排列，串成从公元前 3000 年至今的交通史（只讲类型不写型号与品牌；配图占位块直写期望文件名，逐张 AI 配图提示词由 `main.js` 派生到 `_dev/IMAGE_PROMPTS.md` 交图片 agent；16 项渲染自检 + 18 项打包前置校验过） | 已发布 |
| 人类科技树 | `vibeknow/tech-tree/` | 仿文明系列科技树：72 项技术 / 10 个时代分层排开，120 条「前置科技」连成 DAG——同代与相邻时代实线、跨多代虚线，节点按前置重心自动排序减少交叉；时代是左侧吸顶立柱（不横占节点区，连线全程可见），纸面编年史风格；点节点反色并点亮整条脉络、可逐级跳转，另有图鉴与详情两视图；顶部按容器宿主按钮行显式预留 50px / 内嵌 70px 预留带、**标题居中放在这条带子中间**（对齐 `vibegame/ai-os` 写法，小屏不换行且省一行高度）；BGM 用《Sogno di Volare》105s 乐句，base64 藏在 `.js` 里经 Web Audio 播放（容器不收音频扩展名、CSP 禁 data:/blob:，与 `jurassic-park-3d` 同一结论），解码后 0.80 MiB 卡在 1 MiB 门禁内；详情页可分享到小红书（原图经 Canvas → data:uri → `writeTempFile` → `postNote`，正文即科技介绍；因设备能力清单禁用网络请求 API，取图不走 XHR）（数据真源 `data.js`，82 张配图已就位且逐张核过尺寸体积，提示词由脚本派生，43 项渲染自检 + 33 项打包前置校验过） | 完整（zip + 配图）· BGM 版权待清 |
| 中国铁路发展图鉴 | `vibeknow/china-rail-atlas/` | 中国铁路车型 / 车站图鉴：火车头 20 + 客运车厢 10 + 货运车厢 10 + 著名车站 10 = 50 条目 + 4 张分类封面，写**具体型号**不写泛类，图鉴式**正侧视（车辆）/ 正立面（车站）**插画；Wikimedia Commons 真实照片已落盘 133 张作生图参考、逐条外观要点已提炼进 `_dev/ref-index.md`，配图提示词由 `main.js` 派生到 `_dev/IMAGE_PROMPTS.md`；单文件 H5（列表 + 分类 tab + hash 路由详情页 + 占位块常驻） | 框架完成 · 参考图就位（133 张）· 配图待生 |
