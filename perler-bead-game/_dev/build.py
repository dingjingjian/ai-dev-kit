...
:root{
  ...
  /* 安全区：默认跟随系统；in-app 时额外避开宿主 App 的顶部返回/关闭按钮 */
...
body.in-app{
  --safe-top:calc(env(safe-area-inset-top,0px) + 40px);
  --safe-l:48px;
  --safe-r:92px;
}
...
// 宿主 App（如小红书小工具）顶部自带返回/关闭按钮，需额外让出安全区
function applySafeArea(){
  try{
    const q=new URLSearchParams(location.search);
    const ua=navigator.userAgent||'';
    if(q.get('inapp')==='1'||q.get('embed')==='1'||/xiaohongshu|redbook|xhslink/i.test(ua)){
...
with open(out_path, 'w', encoding='utf-8') as f:
    f.write(html)
print('generated', out_path, 'chars', len(html))
