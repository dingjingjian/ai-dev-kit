...
前置校验 -> 构建 dist（index.html + main.js）-> 压缩 dist 的"内容"。
...
ROOT = Path(__file__).resolve().parent.parent          # offwork-heatmap/
DIST = ROOT / "dist"
ZIP = ROOT / "offwork-heatmap.zip"
...
GUARDS = [
    ...
    # device-capabilities.md §4：禁止 a[download] / blob 下载
    ("禁用能力：无 a[download] / blob 下载", not re.search(
        r"download\s*=|createObjectURL", js)),
    # 容器内 <input type=file> 只能选图片，不能用于导入 JSON
    ("无 <input type=file> 文件选择", "<input type=\"file\"" not in html and
     "<input type='file'" not in html),
]
print("—— 打包前置校验 ——")
