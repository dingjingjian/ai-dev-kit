...
- **数据自管**：按小红书 §2.4 / §3.7 双通道存储——客户端 ≥ 9.46.0 且注入 SDK 时用 Storage JS API（`window.xhs.miniTool.setStorage`）持久化，同时**始终再写一份 `localStorage` 镜像**；读时容器优先、本地兜底并回填，升级 / 降级后两侧自动对齐，任一通道写成功就不算丢数据。设置面板「数据管理」下方只写当前实际走的通道，用户侧不出现判定细节。支持导出 / 导入 JSON 备份、一键清空。
...
- **存储双通道（§2.4 / §3.7）**：客户端 ≥ 9.46.0 且注入 `setStorage`/`getStorage` 时启用容器 Storage 端能力（推荐，持久化有保障）；`localStorage` 作为镜像通道**始终同步写一份**（容器不保证持久性，仅作兜底）。启动时按 §3.6 读 `buildVersion` 判定容器通道是否可用，可用则后台把两侧对齐（容器有本地没有 → 补本地；本地有容器没有 → 迁进容器；都有以容器为准），升级 / 降级都不丢数据，且**不删除任何一份**。端能力调用一律走 `success`/`fail` 回调 + 800ms 超时兜底（旧容器上 Promise 版不可靠，也不能挂住调用方），失败退回镜像通道；写入返回 `Promise<boolean>`，**只有两条通道都失败**才 toast 提示（§3.7 要求调用方处理 false）。
- **容器通道判定不靠「一次快照」（真机踩坑加固）**：`window.xhs` 的注入时机官方未承诺、`launchOptions.miniToolEnv.buildVersion` 也可能缺失，所以 ① 首屏脚本跑得比注入早时，先用镜像通道起步，后台按 100ms 轮询等 SDK 最多 1.5s，等到即切到容器通道并补做对齐（已打过卡则把内存最新值上推，绝不让容器回灌旧值），**不必重开小工具**；② `getLaunchOptions` 两种形态都给（`success`/`fail` 回调 + Promise 返回值，再叠 800ms 超时），旧容器只认回调也能取到版本号；③ 打开设置时若仍是降级态，会再补判一次（最多等注入 300ms），不让结论停留在首屏那一瞬间。
