# -*- coding: utf-8 -*-
"""
小工具 ZIP 打包（含 echarts.min.js）。
修复一律直接改 index.html / main.js 后重跑本脚本。

前置校验 -> 构建 dist（index.html + main.js + echarts.min.js）-> 压缩 dist 的"内容"。
...
ROOT = Path(__file__).resolve().parent.parent          # echarts-gallery/
DIST = ROOT / "dist"
ZIP = ROOT / "echarts-gallery.zip"
...
echarts_path = ROOT / "echarts.min.js"

# ---------- 前置校验：不合规直接拒绝打包 ----------
GUARDS = [
    ("脚本已外置，无内联 <script>", not re.search(r"<script(?![^>]*\bsrc=)[^>]*>", html, re.I)),
    ("index.html 引用 ./echarts.min.js", '<script src="./echarts.min.js"></script>' in html),
    ("index.html 引用 ./main.js", '<script src="./main.js"></script>' in html),
    ("echarts.min.js 存在", echarts_path.exists()),
...
for name in ("index.html", "main.js", "echarts.min.js"):
