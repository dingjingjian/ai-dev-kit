...
按赛道分组，赛道定义与维护约定见 [`TRACKS.md`](TRACKS.md)。目录结构不随分类变动。

### #vibetool　实用工具

| 项目 | 目录 | 说明 |
|------|------|------|
| 拼豆设计工具 | `perler-bead-designer/` | 浏览器端拼豆图案设计，行业标准色卡、钉板形状、用料统计、图纸导出 |
| 像素画编辑器 | `pixel-art-editor/` | 多种画笔、图层、撤销重做，导出 PNG / SVG |
| Word 转 Markdown | `word-to-md/` | Word 文档转 Markdown 格式工具 |
| 火箭发射 | `rocket-launch/` | 火箭发射主题小工具（2D），含构建/打包/校验脚本 |
...
| AI 计算器 | `ai-calculator/` | 仿真计算器，随机产生计算错误，判断对错得分，锻炼心算验算 |
...
| 项目 | 目录 | 说明 |
|------|------|------|
| 3D 地球科普 | `earth-3d/` | WebGL 展示昼夜交替、四季成因、地球内部结构与经纬网格 |
| 太阳系 3D | `solar-system-3d/` | WebGL 渲染行星、轨道与土星环，支持点击追踪天体 |
| 3D 火箭发射 | `rocket-launch-3d/` | 3D 火箭发射模拟，支持展示/拆解等模式，含遥测与倒计时 |
| 3D 月球科普 | `moon-3d/` | 高细节月球，演示月相变化与潮汐锁定等科学知识 |
| 分子结构 | `molecule/` | 分子空间构型可视化（含构建/打包/校验脚本） |
...
| 网站截图 | `screenshot/` | 多网站批量截图采集，自动处理懒加载/Cloudflare/字体 |
| 密码管理器 | `password-manager/` | AES 加密本地密码库，自动备份，Web 与 Electron 双模式 |
