...
| 博主 | 工具名 | 类型 | 原文链接 | 亮点 | 归档日期 | 目录 | 画廊发布 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
## 与画廊（仓库根 `../vibecoding-gallery/`）同步：两阶段机制
...
**本目录就是画廊的「蓄水池」**：日常只归档、不碰画廊；攒够一批（如 5~10 件）再批量发布进画廊。上方「总索引」表格的「画廊发布」列记录每条的发布状态（⬜ 待发布 / ✅ 已发布）。

### 阶段一 · 积累期（日常，零风险，只动本目录）
看到优秀作品就按上面规范归档，只做三件事：
1. 建 `<博主名-工具名>/` + `screenshots/cover.*`；
2. 填子目录 `README.md`（**必须含完整原文链接含 `xsec_token`**、简介、亮点）；
3. 在本文件「总索引」追加一行，并把「画廊发布」列留空（⬜）。

> ⚠️ 此阶段**不要**改 `vibecoding-gallery/` 任何文件（ORDER / main.js / 打包都不动）。
> `xsec_token` 有时效，归档时务必贴当时能打开的完整链接；取不到令牌的在 README 标注，发布期照常保留、详情页标「未取到分享令牌」。

### 阶段二 · 发布期（攒够 N 件后，一次性批量）
```bash
