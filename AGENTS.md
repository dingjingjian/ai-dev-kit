...
本目录是一个 AI 开发工具集合，包含多个独立子项目。多数浏览器端工具为零依赖单文件 HTML，开箱即用；服务端项目集中在 `_internal/ai_gateway/`、`_internal/password-manager/`、`_internal/daily-report/`、`_internal/ai-news/` 等。
...
按赛道分组，与顶层目录一一对应（`vibetool/` / `vibegame/` / `vibeart/` / `vibeknow/` / `_internal/`）。赛道定义与判定标准见 [`TRACKS.md`](TRACKS.md)。
...
| 火箭发射 | `vibetool/rocket-launch/` | 火箭发射主题小工具（2D），含构建/打包/校验脚本，生成可分发压缩包 |
...
| 网站截图 | `_internal/screenshot/` | 多网站批量截图采集，自动处理懒加载/Cloudflare/字体，分类输出 |
| 密码管理器 | `_internal/password-manager/` | 本地密码管理器，AES 加密存储，自动备份，支持 Web 和 Electron 双模式 |
...
- **赛道归属看目录 + `TRACKS.md`**：项目位于 `vibetool/` `vibegame/` `vibeart/` `vibeknow/` 哪个目录下即属哪个赛道，不参赛的收在 `_internal/`。换赛道用 `git mv` 迁移，并同步更新 `TRACKS.md`（唯一真源）与项目 README 顶部的赛道行。
- **脚本与产物分离**：构建/打包/校验脚本（如 `vibeknow/molecule/`、`vibetool/rocket-launch/` 下的 `*.mjs`/`*.ps1`）与生成产物（压缩包、导出文件）应分目录管理，避免污染源码。
