...
本目录是一个 AI 开发工具集合，包含多个独立子项目，按小红书 vibecoding 四赛道分目录组织。多数浏览器端工具为零依赖单文件 HTML，开箱即用；服务端项目集中在 `vibetool/ai_gateway/`、`vibetool/password-manager/`、`vibetool/daily-report/`、`vibetool/ai-news/` 等。
...
按赛道分组，与顶层目录一一对应（`vibetool/` / `vibegame/` / `vibeart/` / `vibeknow/`）。赛道定义与判定标准见 [`TRACKS.md`](TRACKS.md)。
...
- **赛道归属看目录 + `TRACKS.md`**：项目位于 `vibetool/` `vibegame/` `vibeart/` `vibeknow/` 哪个目录下即属哪个赛道。换赛道用 `git mv` 迁移，并同步更新 `TRACKS.md`（唯一真源）与项目 README 顶部的赛道行。
