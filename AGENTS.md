...
按赛道分组。**赛道归属以 [`TRACKS.md`](TRACKS.md) 为唯一真源**；目录结构不随分类变动（分类是元数据，不是存储结构）。
...
| AI 计算器 | `ai-calculator/` | 仿真计算器，随机产生计算错误，判断对错得分、连对加成，锻炼心算验算能力 |
...
| 3D 火箭发射 | `rocket-launch-3d/` | 3D 火箭发射模拟，WebGL 渲染，支持展示/拆解等模式，含遥测与倒计时 |
| 3D 月球科普 | `moon-3d/` | 3D 月球科普展示工具，WebGL 渲染高细节月球，演示月相变化与潮汐锁定等科学知识 |
...
- **赛道归属改 `TRACKS.md`**：新增或调整项目赛道时，只改 `TRACKS.md`（唯一真源）与该项目 README 顶部的赛道行，**不要为分类物理移动目录**——会打断跨项目资源引用（如 `moon-3d` 复用 `solar-system-3d` 的月面纹理）与构建脚本路径。
