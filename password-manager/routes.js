// ============================================================
// 共享路由模块 - 供 main.js(Electron)与 server.js(Web)复用
// 消除两处路由定义的重复,统一维护
// ============================================================
...
/**
 * 创建并配置 Express 应用
 * @returns {{ app: Express, setAuthenticated: Function }}
 */
function createApp() {
  const app = express();
  ...
  app.post('/api/backups/restore', requireAuth, (req, res) => {
    ...
    if (!backupFile) return res.status(400).json({ error: '请指定备份文件' });
  ...
  return { app, setAuthenticated: (v) => { isAuthenticated = v; } };
}
