/*
 * air-tycoon — geo.js
 * 经纬度数学 / 大圆航线 / 球面插值 / 航线距离
 * 依赖：data.js（仅用 AT.CONFIG.earthRadiusKm，无 THREE 依赖，可无头断言）
 * 命名空间：window.AT（经典脚本，无 import/export/type=module）
 *
 * 本文件复用自 vibegame/defcon/src/geo.js（原用于弹道/溯源），
 * 民航题材只用到其中的大圆几何（distKm / slerpLL / gcPoint / ll2v / v2ll / centroid），
 * 弹道坡度 ballistic / 溯源 reverseTrace / offsetLL 保留但本作不使用 ——
 * 保留是为了「航线弧线抬升」与「飞机沿弧线飞行」能直接复用 gcPoint 的球面插值，
 * 以及未来若要扩展「航班绕行/改航」时有现成的球面工具。 */
(function (global) {
  'use strict';
  ...
  // t 允许超出 [0,1]：slerp 会沿大圆继续外推（用于「取前方一点求切向」的采样，
  // 见 legAt 的 fwd —— 两端不外推会让切向退化为零向量）。
  ...
  /* 往返航段的「相位 → 弧上位置 + 行进方向」。
   *
   * 飞机在 a↔b 之间往复，一个周期相位 u∈[0,1)：
   *   u < 0.5  去程 a→b（位置 t 由 0 增到 1）
   *   u ≥ 0.5  回程 b→a（位置 t 由 1 减到 0）
   * 返回 { t, fwd }：t 为弧上位置（三角波，恒在 [0,1]），fwd 为 +1 / -1。
   *
   * ⚠ 为什么要把这件事单独抽成一个函数、而不是在渲染层就地写：
   *   位置 t 在一个周期内被**经过两次**（去程一次、回程一次），所以
   *   「t < 0.5 ? 去程 : 回程」是错的 —— 它把「在弧的哪一半」当成了
   *   「往哪边飞」。air-tycoon 首版正是这么写的，实测导致**每个航段有
   *   一半时间飞机倒着飞**（探针 tools/probe-heading.js 量到 49.9% 相位）。
   *   行进方向只能由**相位**推出，与位置无关。
   *
   * 抽成纯函数后可以放在 headless 里断言（见 tests/headless.js「往返航段」），
   * 不必依赖 THREE 与相机就能把这条不变量钉住。 */
  function legAt(u) {
    var p = u % 1;
    if (p < 0) p += 1;                    // 负相位环绕保护
    var outbound = p < 0.5;
    return {
      t: outbound ? p * 2 : (1 - p) * 2,
      fwd: outbound ? 1 : -1
    };
  }
