...
:root{
  ...
  /* 顶部强制留白：小红书 iframe 里 env(safe-area-inset-top) 恒为 0，
     顶栏会直接顶在屏幕最上沿（被宿主 UI / 小工具自带按钮压住）。
     用一条显式常量留白，顶栏整体下移 —— 上方露出背景，不再是一整块面板。
     改这一个值就能调，body / .overlay 都跟着它走。（参照 perler-city 的 --top-gap 机制） */
  --top-gap:50px;
...
/* 小红书宿主内嵌：左右安全区给宿主按钮预留空间。
   顶部预留已由 :root 的 --top-gap 无条件承担，不再依赖 body.in-app 被注入。 */
...
<body>
  ...
  <div class="topbar">
    ...
    <div class="title">
      <div class="main" id="patName">子鼠</div>
      <div class="sub" id="patMeta">入门 · 17×17 · 150 豆</div>
    </div>
    <div class="rec">
  ...
  <div id="homeView">
    <div class="home-head">
      <div class="home-seal">肖</div>
      <div class="home-title">
        <div class="m">十二生肖拼豆坊</div>
        <div class="s">十二生肖 · 对图填豆 · 导图线下拼</div>
      </div>
      <div class="home-stat">已完成<b id="homeDone">0</b><span id="homeTotal">共 12 幅</span></div>
    </div>
...
const SFX=(function(){
  ...
  const api={
    ...
    load:function(){
      try{
        var a=JSON.parse(localStorage.getItem('pzd_audio')||'null');
        if(a){ on=a.s!==0; bgmOn=a.b!==0; }
      }catch(e){}
      api.syncLabels();
    },
    save:function(){
      try{ localStorage.setItem('pzd_audio',JSON.stringify({s:on?1:0,b:bgmOn?1:0})); }catch(e){}
    },
    syncLabels:function(){
...
// 每幅图纸的独立进度：pzd_progress = {图纸id:{g:压缩棋盘,t:已用时,m:步数,k:失误}}
function loadProgress(){
  try{progress=JSON.parse(localStorage.getItem('pzd_progress')||'{}')||{};}catch(e){progress={};}
}
...
function saveProgress(){
  ...
  try{localStorage.setItem('pzd_progress',JSON.stringify(progress));}catch(e){}
}
...
// 本地存档：pzd_records = {图纸id:{t用时秒,s星级,m步数}}（兼容旧版 pzd_completed 数组）
function loadRecords(){
  records={};
  try{
    const raw=JSON.parse(localStorage.getItem('pzd_records')||'null');
    if(raw&&typeof raw==='object'&&!Array.isArray(raw)) records=raw;
    else{
      const old=JSON.parse(localStorage.getItem('pzd_completed')||'[]');
...
function saveCompletion(){
  ...
  try{localStorage.setItem('pzd_records',JSON.stringify(records));}catch(e){}
...
function renderExportSheet(){
  ...
  g.fillText('十二生肖拼豆坊 · '+p.name,X0,36);
  ...
  g.fillText(p.name.slice(-1),W-pad-35,40);
  ...
  g.fillText('照色号逐格放豆 · 拼完对照复核再熨烫 · 十二生肖拼豆坊',X0,H-18);
