...
(async () => {
  ...
  // 从 main.js 解析「子鼠」目标图案，按正确颜色逐格落豆（避免用四舍五入的进度当判据）
  const jsSrc = require('fs').readFileSync(path.resolve(__dirname, '..', 'main.js'), 'utf8');
  const blk = jsSrc.match(/"id":\s*"rat"[\s\S]*?"grid":\s*\[([\s\S]*?)\]/);
  ...
  console.log('首页卡片导出按钮   :', expBtnOnCard, expBtnOnCard === 12 ? '✅ 12 张卡全配齐' : '⚠');
  ...
  const ok = real2.length === 0 && !inlineScript && !inlineEvent && filled > 5000 && toolBtns > 0 && swatches > 0
    && expShown && expReady && expBtnOnCard === 12;
  console.log('\n结论:', ok ? '✅ 冒烟通过' : '❌ 存在问题');
  dom.window.close();
  process.exit(ok ? 0 : 1);
})();
