...
(async () => {
  ...
  // 从 main.js 解析「明月」目标图案，按正确颜色逐格落豆（避免用四舍五入的进度当判据）
  const jsSrc = require('fs').readFileSync(path.resolve(__dirname, '..', 'main.js'), 'utf8');
  const blk = jsSrc.match(/"id":\s*"moon"[\s\S]*?"grid":\s*\[([\s\S]*?)\]/);
  ...
  console.log('胜利弹窗已弹出     :', winShown, winShown ? `(星级 ${winStars})` : '');

  // —— 中秋活动功能：猜灯谜（拼成 1 幅 → 第 1 盏已解锁） ——
  console.log('\n—— 活动功能：猜灯谜 ——');
  click(doc.getElementById('riddleBanner'));
  await new Promise((r) => setTimeout(r, 300));
  const riddleShown = /show/.test((doc.getElementById('sheetRiddle') || { className: '' }).className || '');
  const opts = doc.querySelectorAll('.rd-opt').length;
  const qText = ((doc.querySelector('.rd-q') || {}).textContent || '').trim();
  console.log('灯谜面板已打开     :', riddleShown);
  console.log('当前谜面           :', qText ? qText.slice(0, 18) + '…' : '(无)');
  console.log('选项数量           :', opts, opts === 4 ? '✅' : '⚠');
  if (opts === 4) {
    click(doc.querySelectorAll('.rd-opt')[0]);          // 全部谜底 A 选项正确答案看数据，逐个试
    await new Promise((r) => setTimeout(r, 200));
    let okIdx = [...doc.querySelectorAll('.rd-opt')].findIndex((o) => /ok/.test(o.className));
    if (okIdx < 0) {
      // 首个不对就清掉 bad 再逐个点
      for (let i = 1; i < 4; i++) {
        click(doc.querySelectorAll('.rd-opt')[i]);
        await new Promise((r) => setTimeout(r, 120));
        okIdx = [...doc.querySelectorAll('.rd-opt')].findIndex((o) => /ok/.test(o.className));
        if (okIdx >= 0) break;
      }
    }
    const rb = (doc.getElementById('rbState') || {}).textContent || '';
    console.log('答对一题点亮灯笼   :', okIdx >= 0 ? `✅ (正确项 ${'ABCD'[okIdx]}，${rb.trim()})` : '⚠ 未通过');
  }
  click(doc.querySelector('#sheetRiddle .sheet-close'));
  await new Promise((r) => setTimeout(r, 200));

  // —— 收尾交互 ——
  click(doc.getElementById('winReplay') || doc.body);            // 重玩（若弹窗已关则跳过）
  await new Promise((r) => setTimeout(r, 300));
