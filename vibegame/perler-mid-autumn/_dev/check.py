...
用法：
  ...
  python _dev/check.py moon 月饼    按 id 或中文名打印像素网格 + 行宽序列（验圆度/对称性）
  python _dev/check.py              默认打印 明月 / 月饼 / 柚子 / 莲蓉蛋黄
...
def show(key):
    ...
    print('  上下等宽:', 'OK' if mirror else 'NOTE 有额外元素（叶/云/梗）或本身不对称')
...
if __name__ == '__main__':
    ...
    if args and args[0] == 'pal':
        ...
        for k in (args or ['moon', 'mooncake', 'pomelo', 'cake_cut']):
            show(k)
