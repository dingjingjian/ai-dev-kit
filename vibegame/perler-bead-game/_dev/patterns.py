# -*- coding: utf-8 -*-
"""国风拼豆坊 · 图案稿：20 幅国风纹样 + 预览渲染。

运行：python _dev/patterns.py
产出：_dev/patterns.json（游戏数据）、_dev/preview.png（目视校验用预览图）
体检：python _dev/check.py pal（色卡）、python _dev/check.py 铜钱（网格/行宽）
"""
import math, os, json
...
# 相对首版修掉四处难区分：银灰↔藕荷（38→88）、栗棕↔绛紫（33→62）、
# 赭石↔描金（54→75）、藤黄↔描金（38→114）。
PAL = {
    'w': '#F5EFE2',  # 月白
    'e': '#A8A292',  # 银灰
    'k': '#221E1B',  # 墨黑
...
class G:
    ...
    def ring2(self, cx, cy, r_out, r_in, c_out, c_in=None):
        ...
        比 ring() 可靠两处：
        1) 外沿形状由 r_out 精确决定，圆才会圆；ring() 用「中心半径 r + 环宽 w」
           描述，改半径时容易把外沿一起带偏。
        2) 环带不会在某些行整行消失——两个实心圆相减在任何行都至少留下一格，
           而落在区间外的整行在 ring() 里会被静默丢掉。
        半径差建议 >= 1.0，否则个别行仍可能无环带格子。
        c_in 省略时保留内圆原有像素（纯相减）；给出则用该色填满内圆。
        """
...
def p_knot():
    ...
    for dx, dy in [(0, -4.6), (0, 4.6), (-4.6, 0), (4.6, 0)]:
        g.ring2(C + dx, C + dy, 1.975, 1.025, 'r')
