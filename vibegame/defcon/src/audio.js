...
(function (global) {
  ...
  var VOICES = {
    ...
    /* ── 核爆 nuke（重构版）──────────────────────────────────
     * 语义：核爆炸 —— 极重的低频冲击 + 撕裂性的中频 + 超长的隆隆余韵。
     *   ① 起爆冲击  0.12 s  超宽频瞬态（300→2200 Hz）—— 「炸」的那一下
     *   ② 主轰鸣    1.75 s  锯齿 90→30 Hz 经低通 —— 核爆的核心重量，缓慢下沉
     *   ③ 撕裂层    0.60 s  带通噪声 1200→400 Hz —— 「核」特有的破坏质感
     *   ④ 长尾回响  2.40 s  低频噪声 200→70 Hz —— 隆隆远去，制造规模与空间
     *
     * 关键在**衰减速度**：② 用慢衰减（1.7 s）而不是快衰减 ——
     *   慢衰减的听感是「轰鸣」，快衰减才会变成「一记打击」。
     *   历次被判「像在打鼓」的根因就在这里：把低频轰鸣的包络收得太快，再一抬电平，
     *   听感就从「远处沉下去」变成「近处砸一下」。这一版把包络拉长到 1.7 s，
     *   既保留了重量，又彻底离开「打击」的听感区。
     *
     * 走线：整条走 nukeBus 绕开压缩器 —— 一段持续两秒多的低频被压缩器压平就不像爆炸了。
     * 手机外放：③④ 落在 70~1200 Hz，是外放能抓到「这是核爆」的部分；② 负责「沉」，
     *   耳机 / 桌面上那层推不动的重量感全来自它。 */
    nuke: function (t, _a, o) {
      var g = G(o), m = M(o);   // MAX_STACK.nuke = 0，m 恒为 1；保留形参是为了与其它音效同构
      // ① 起爆冲击：超宽频瞬态，给出「炸」的起点
      filteredNoise(t, 0.12, 0.22 * g, 'bandpass', 300 * m, 2200 * m, 0.6, nukeBus);
      // ② 主轰鸣：低频下沉 90→30 Hz，核爆的核心重量（慢衰减 = 轰鸣，不是打击）
      var o1 = ctx.createOscillator();
      o1.type = 'sawtooth';
      o1.frequency.setValueAtTime(90 * m, t);
      o1.frequency.exponentialRampToValueAtTime(30 * m, t + 1.6);
      var lp = ctx.createBiquadFilter();
      lp.type = 'lowpass';
      lp.frequency.setValueAtTime(400, t);
      lp.frequency.exponentialRampToValueAtTime(90, t + 1.6);
      var g1 = envelope(t, 0.015, 1.7, 0.32 * g);
      o1.connect(lp); lp.connect(g1); g1.connect(nukeBus);
      o1.start(t); o1.stop(t + 1.75);
      // ③ 撕裂层：中频噪声，快速衰减，给「核」的破坏质感
      filteredNoise(t, 0.60, 0.17 * g, 'bandpass', 1200, 400, 0.8, nukeBus);
      // ④ 长尾回响：低频轰鸣缓慢远去，制造规模
      filteredNoise(t + 0.15, 2.40, 0.13 * g, 'bandpass', 200, 70, 0.6, nukeBus);
    },
