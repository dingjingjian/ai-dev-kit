...
- **月球天气**（点主屏天气小组件进入，DESIGN.md §4.9）：由 `vibeknow/moon-3d` 整合而来，**整页只有天气**（无分段切换/滑块/设置/科普块）。渲染**直接采用 moon-3d 的 Three.js 场景**：本地 `./assets/three.min.js`（与 `vibeknow/earth-3d` 同款引入方式，零 CDN）+ WebGL/sRGB/ACES、星空天球、双层辉光、原版月面贴图 `./assets/moon.jpg`；**月相按当前时间推算**（平均朔望月 + 基准新月，0° 新月 / 180° 满月）驱动太阳方位角，**不自转**，只保留拖动旋转与滚轮/双指缩放；**月面正面（月理 0° 经度）朝地球**——贴图经度基准与 `SphereGeometry` 的 UV 差 90°，由固定偏转 `WX_FACE_YAW = −π/2` 校正（默认视角正对正面中心、东右西左同地面观测；`AIOS.weatherState().faceYaw` 供自检，别删）；无 WebGL / 上下文丢失时观测台降级提示、读数照常。
...
- **存储**（对照小红书容器能力清单 §2.4 / §3.6 / §3.7）：**容器 Storage JS API 优先**（客户端 ≥ 9.46.0，`setStorage` / `getStorage` / `getStorageInfo`），`localStorage`（前缀 `aios_`）为低版本镜像/降级通道；启动时把 `aios_*` **并发**水合进内存缓存（每个 key 各自 800ms 超时，与 1.8s 开机动画并行），业务读写仍是同步的。写入**双通道落盘**（镜像同步先行、容器异步跟上，任一成功即算成功）、读时以容器值为准，两侧缺哪边补哪边且不删除任何一份，客户端升降级都不丢数据；版本判断忽略 `buildVersion` 末 3 位、逐级判空 + `getLaunchOptions` 异步兜底，容器异常时最多等一个超时周期即退回镜像通道，不阻塞启动。设置「关于本机 → 存储方式」显示当前通道与用量 —— **只报通道名、不缀「降级」**（容器不可用时就是 `localStorage`），只有容器写入失败时才如实标注已回落 `localStorage`。本层与 `vibetool/offwork-heatmap`、`vibetool/mood-diary` **同构**（统一存储契约），三处改动请同步。
- **容器通道判定不靠「一次快照」（真机踩坑加固，三处同款）**：`window.xhs` 的注入时机官方未承诺，所以 ① 启动时若 SDK 还没注入，先用镜像起步（开机动画不等它），后台按 100ms 轮询最多等 1.5s，等到即切到容器通道并补做水合（容器为准、缺哪边补哪边），**不必重开小工具**；② `getLaunchOptions` 两种形态都给（`success`/`fail` 回调 + Promise 返回值，再叠 800ms 超时），旧容器只认回调也能取到版本号；③ 每次进设置页补判一次（最多等注入 300ms）。开机阶段自身也会写 `mode` / `wall`（`applyTheme`），那不是「用户改过」—— 只有开机流程走完后的写入才记 dirty，否则迟到判定会把开机默认值反推上去盖掉容器里的真值；真记了 dirty 则只上推、不回灌。
- **判定细节只给开发，不上屏**：「关于本机 → 存储方式」永远是通道名（+ 用量），不解释为什么。原因码、真机原始 `buildVersion`、端能力里缺哪个接口 / 实际注入了哪些，一律 `console.log('[storage] …')`；需要上屏时给 URL 加 `?debug`（或 `#debug`）。排查口诀：`?debug` 里若 `buildVersion` 低于 `9460xxx`，是**客户端版本没到 9.46.0**（Storage 这批接口随该版本才注入），此时退到 `localStorage` 是文档规定的预期行为。
- **系统音效**（DESIGN.md §4.11）：全系统统一音效层 —— **Web Audio 实时合成 25 个系统语义音效，零音频文件**（不占包体、不引外部资源）；命名即语义（`tap` / `open` / `ok` / `wrong` / `shot` / `alarm` …），同一语义在任何应用里听起来一致；触发走文档级 `click` 委托 + `data-sfx` 标注（判定 / 快门 / 挂断等由动作发声，不叠通用点击音，同音效 30ms 去重）；**首个手势前不建 AudioContext**（躲开自动播放策略与告警），无 Web Audio 时整层静音降级、交互照常；设置「声音与触感 → 声音」可整体静音（`aios_sound` 持久化）—— 原「系统音效」行与 v1 的「声音」吐槽行已**合并为这一行**（开关即音效总开关），另存 v1 的「触感」吐槽行；设备身份（关于本机：FakePhone 18 NoDuo / 人工智能 OS v2.0 / AI引擎 全靠人工 v0.2 / 存储方式）见 DESIGN.md §4.13。
- **应用商店**（DESIGN.md §4.12）：主屏第 7 格的应用，介绍**整个 ai-dev-kit 仓库已开发的小工具**（不是本机已装的 9 个应用），按 `TRACKS.md` 的四分类分组、可分类筛选，**纯展示不跳转**（容器内无法打开别的项目）。清单**构建期由仓库根 `TRACKS.md` 派生**：`_dev/build.py` 的 `store_apps()` 解析分类表后注入 `main.js` 的 `__STORE_DATA__`（中文不转义、跳过 `.skill/`），与小工具包体无关、运行时零外部读取 —— 与相机取景清单同属「构建期派生、不手抄」（手抄必然与上游分叉）。**每项在构建期折成商店口径**：副标题由定位折（`store_tagline()` 砍掉技术括注与上游路径，如 ai-os 那条的 `vibeknow/moon-3d`）、上架标签由物料状态折（`store_state()` → 已上架 / 开发中 / 内部自用）、**第三方软件 / 平台名由 `store_neutral()` 换通用说法**（飞书写入→在线文档写入、飞书多维表格→在线表格、GitHub 式→格子式、Cloudflare→反爬校验、Word 转 Markdown→文档转 Markdown、乐高 10294→积木、Blender 建模→3D 建模、Electron→桌面端、Git 提交→提交记录；名称与副标题都过这一层，`TRACKS.md` 原文不动）；**目录路径与物料状态原文不上屏**（那是仓库索引的字段，不是商店的文案），卡片一行一卡不展开。数据文本一律 `esc()` 转义后进 `innerHTML`。
- **自检**：`_dev/smoke_test.py`（playwright 无头）断言导航栈 / 安全区 / 顶部净空 / 计算器固定页（内容区不可滚动、判定阶段页脚不跳动、小屏键盘贴底、正常净高不产生滚动条、**极矮净高（横屏 / 被压扁的容器）键盘行高有下限且整页可滚动到「=」**）/ **月球天气（小组件入口、单画布 WebGL、贴图 data-tex=ok、本地 three.min.js、仅天气、月相读数与测试独立复算对拍、静置 900ms 画面字节不变即不自转、拖动改 theta、滚轮改 radius、固定时钟验满月与蛾眉月两种相位画面不同）** / **相机（取景图为包内 `./assets/cam/food-*.webp`、已解码上屏、铺满取景框、3s 换图且不与上一张重复、无预览/重拍/保存控件、拍完即换下一张、连拍升温、过热转红与封锁快门、封禁时按不动、静置散热后恢复）** / **存储通道（容器 Storage、版本门槛、降级兜底、迁移一致性）** / **系统音效（词表齐备、首个手势前不建上下文、点击·判定·快门发声计数、静音后计数不涨且跨会话保持、无 Web Audio 时降级不报错）** / **设置页文案（卡片逐行对拍：「声音与触感」只剩「声音」「触感」两行且开关就在「声音」行内；关于本机 设备名称 / 系统版本 / AI引擎 / 存储方式 各行精确相等，存储方式只写通道名且无「降级」字样）** / **应用商店（与测试自带的独立 `TRACKS.md` 解析及两套折算规则复算三方对拍：分类名与顺序、逐项 分类+名称+目录+定位+状态全量续存、副标题与上架标签折算一致、卡片数=项目数、`.skill/` 零命中、**目录路径与状态原文零上屏**、招牌图标相邻不同色、本机标「已安装」、**上屏文案零第三方软件名（按品牌词表扫整页视图文本）**、分类筛选只剩该分类且计数正确、切回全部恢复）**，并扫描 JS/CSS 超基线语法；回归截图输出到 `_dev/_shots/`（临时，不入库）。整套耗时约 2–4 分钟：闹钟用例要等响铃窗口对齐到 `:00`/`:30`，窗口恰好已过期时会多等一个刻度周期。
