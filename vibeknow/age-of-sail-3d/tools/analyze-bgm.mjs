 ...
 *
 * 本文件在 age-of-sail-3d / jurassic-park-3d 两个项目的 tools/ 下各存一份、内容一致
 * （每个项目都要独立可打包，所以工具随包各带一份；改一处请同步另一处）。
 *
...
for (const c of cands) {
  if (c.r == null) { const p = pcmCorr(c.tA, c.tB, 3, 100); c.r = p.r; c.lag = p.lag; }
  c.total = c.score + (c.r > 0.5 ? 0.15 : 0);
}
...
console.log('');
console.log('② 用 make-bgm 截段、buffer 就是乐句（如 jurassic-park-3d 的做法）：');
