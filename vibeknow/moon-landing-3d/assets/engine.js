(function (global) {
  ...
  var PHONG_FS = [
    ...
    ' vec3 baseCol=uColor;',
    // 通用贴图天体（月球）：非地球、非云层但带贴图时直接采样，轻微提亮补偿低反照率
    ' if(uUseTex>0.5 && uIsEarth<0.5 && uIsCloud<0.5){',
    '   baseCol=texture2D(uTex,vUV).rgb*1.12;',
    // 近月面细节：贴图分辨率有限，着陆近景混入高频噪声模拟月壤/环形山颗粒
    '   float mnf=1.0-smoothstep(40.0,360.0,vDist);',
    '   if(mnf>0.004){ vec3 mp=normalize(vObjPos); float md=noise3(mp*150.0+vec3(2.1,5.3,9.7))-0.5; baseCol*=1.0+md*0.7*mnf; }',
    ' }',
