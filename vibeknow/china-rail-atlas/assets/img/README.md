...
把生成的配图放这里，文件名严格为 `<base>.webp`，base 见
...
1. 用你的生图模型，对着 `_dev/ref/` 的真实照片 + `_dev/ref-index.md` 的「外观要点」生成配图：
   **条目图**（50 张）走图鉴式插画，车辆正侧视、车站正立面；
   **分类封面**（4 张）走写实棚拍渲染，同样使用浅暖白底与正侧视 / 正立面。
   逐张完整提示词见 `_dev/IMAGE_PROMPTS.md`——风格串分两层，不要把两层混用。
