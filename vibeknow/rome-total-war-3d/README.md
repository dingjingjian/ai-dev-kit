...
| **过渡页（即将检阅）** | 标题＝**你选中的阵营名**（副标题＝它的拉丁名，`SPQR · LEGIONVM CODEX` 只留给罗马），底下写队数；一帧画面 + 7.1 秒缓慢推近，走完即进检阅。**画面按阵营换**（⑨ `gate-<key>.webp`，每阵营一张）；还没出图时自动用该阵营横幅顶着，再退通用凯旋门 |
...
| [`docs/场景配图需求.md`](docs/场景配图需求.md) | logo、凯旋门横竖各一帧、兜底内景、大理石、阵营横幅 ×9、**过渡画面 ×9**（共 23 个图位；横幅兼首页 hero 轮播，过渡画面即「即将检阅」页那张） |
| [`docs/背景音乐需求.md`](docs/背景音乐需求.md) | 选曲要求、循环点怎么找、`make-bgm.mjs` 用法 |
| [`docs/素材生成说明.md`](docs/素材生成说明.md) | 生成顺序、后处理、落位、版权留档、踩坑清单 |
| [`docs/提示词包.md`](docs/提示词包.md) | 171 条可直接复制的提示词（**由 `tools/gen_prompts.py` 生成，不要手改**） |
| [`docs/素材质量整改.md`](docs/素材质量整改.md) | 已落位素材的核查结论与整改清单（透明底 / 兵牌画风 / 缺口） |
...
├── tex/                场景图 ×23（外部素材；阵营横幅兼首页 hero 轮播，gate-<key> 是过渡画面）
├── audio/bgm.js        base64 音频（由 tools/make-bgm.mjs 生成）
└── flexgap.js          flex gap 能力检测（兼容基线用）
tools/
├── gen_prompts.py      出图前：从三份规格文档拼出 171 条提示词 → docs/提示词包.md
...
**代码完成 · BGM 已就位 · 主要素材已落位（有两项待整改）。** 图位与提示词已备齐，见 `docs/` 七份文档
（`提示词包.md` 是 171 条可直接复制的成品提示词 —— 写实 ×48、兵牌 ×48、装备 ×52、场景 ×23，
[`素材生成说明.md`](docs/素材生成说明.md) 给出图-落位流程）。

- **已落位**：写实兵种图 ×48、兵牌 ×48、装备 ×52、场景图 ×14、BGM。
- **待整改**（见 [`素材质量整改.md`](docs/素材质量整改.md)）：装备图 ×52 不是真透明底（文件里没有
  alpha 通道）；logo 要按新的**军牌式**规格重出（现版是纯白底、无描边）；
  兵牌 ×48 的画风要按《罗马 2》原版兵牌重出。
- **待补**：⑨ 过渡画面 ×9（`assets/tex/gate-<key>.webp`）。没出之前过渡页用该阵营横幅顶着，功能不受影响。
