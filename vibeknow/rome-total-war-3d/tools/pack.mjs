// ZIP 打包：压缩 index.html + assets/ 为 rome-total-war-3d.zip（zip 根即为 index.html）
...
const out = path.join(root, 'rome-total-war-3d.zip');

// 需要纳入打包的项（相对 root）
const includes = ['index.html', 'assets'];

// 不纳入打包的项（相对 root）：
//   assets/audio/bgm-src.mp3 —— 源曲目，只是构建输入（tools/make-bgm.mjs 读它生成 bgm.js）。
//   容器上传白名单不收任何音频扩展名，放进 zip 会被上传页直接打回；
//   运行时用的是 bgm.js（base64）。
//   assets/audio/README.md —— 素材留档说明，容器不支持 .md 类型。
const excludes = new Set([
  'assets/audio/bgm-src.mp3',   // 抽出来的源曲目（tools/extract-audio.py 产物）
  'assets/audio/bgm.mp3',       // 截段重编码后的乐句（tools/make-bgm.mjs 产物）
