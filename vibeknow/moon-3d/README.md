...
本项目的 Three.js 渲染场景、月面贴图与月相设定已被整合进 [`vibegame/ai-os/`](../../vibegame/ai-os/) 的**「月球天气」**应用（点主屏天气小组件进入）：那条线**直接沿用了本项目的渲染方式**（本地 `three.min.js` + `moon.jpg`，随包分发；sRGB + ACES、星空天球、双层辉光、`setPhase` 同款太阳方位角），并收敛为「只留天气 + 月相按当前时间推算 + 不自转」（自转取消，只保留拖动旋转与缩放）。设定口径见该项目的 `DESIGN.md §4.9`。此处独立版本原样保留。
