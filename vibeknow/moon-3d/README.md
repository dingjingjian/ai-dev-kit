...
本项目的月面贴图与月相设定已被整合进 [`vibegame/ai-os/`](../../vibegame/ai-os/) 的**「月球天气」**应用（点主屏天气小组件进入）：那条线按 ai-os 的 Chrome 61 / ES2017 与「禁用能力零命中、无外部资源」门禁重写为 Canvas 2D 逐像素球面渲染（**贴图沿用本项目的 `assets/moon.jpg`**，构建期转 base64 内嵌；不含 WebGL / Three.js），并收敛为「只留天气 + 月相按当前时间推算 + 不自转」。设定口径见该项目的 `DESIGN.md §4.9`。此处独立版本原样保留。
